...
const elemHeight = elem.offsetHeight
  ...
  // console.log('---tree----', config.tree, config.treeData);
  // Compute the new tree layout.
  const nodes = tree.nodes(treeData).reverse()
  const links = tree.links(nodes)
  if (window.orgChartData) {
    window.orgChartData[config.id] = config.treeData;
  }
  
  console.log('---tree----', config.treeData);
  ...
  const nodeEnter = node
    ...
    // .on('click', onClick.onClick(config))

    // .on('click', () => {
    //   console.log('click');
    // })
    // console.log(object)
  // svg.selectAll('.org-chat-item-collapse')
  // .on('mousedown', onClick.onMouseDown)
  // .on('mouseup', onClick.onMouseUp(config))
  // window.mousedown = onClick.onMouseDown;
  // window.mouseup = () => {onClick.onClick(config)};
  // setTimeout(() => {
  //   svg.selectAll('.org-chat-item-collapse')
  //   .on('mousedown', onClick.onMouseDown)
  //   .on('mouseup', onClick.onMouseUp(config))
  ...
  // debugger;
  // window.svg = svg;
  // console.log('svg', svg.select('.org-chat-item-collapse'))
  // console.log('-------1-----', svg.select('.org-chat-item-collapse'))
  // console.log('-------2-----', svg.selectAll('.org-chat-item-collapse'))
  // Person Card Shadow
  nodeEnter
    .append('rect')
